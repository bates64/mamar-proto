...
export function addFunction(func, sig) {
...
let wasmBinary;if (Module.wasmBinary) wasmBinary = Module.wasmBinary
let noExitRuntime;if (Module.noExitRuntime) noExitRuntime = Module.noExitRuntime
...
function reSign(value, bits, ignore) {
	...
	const half = bits <= 32 ? Math.abs(1 << (bits-1)) // abs is needed if bits == 32
		: Math.pow(2, bits-1)
...
function _fd_seek(fd, offset_low, offset_high, whence, newOffset) {
	try {
		const stream = SYSCALLS.getStreamFromFD(fd)
		const HIGH_OFFSET = 0x100000000 // 2^32
		// use an unsigned operator on low and shift high by 32-bits
		const offset = offset_high * HIGH_OFFSET + (offset_low >>> 0)

		const DOUBLE_LIMIT = 0x20000000000000 // 2^53
